...
/** Read-only snapshot for external evaluation; answers never write back through this endpoint. */
export async function downloadLearningAssessment(kbId: string): Promise<string> {
  const res: any = await get(`/api/v1/learning/kb/${encodeURIComponent(kbId)}/assessment-freeze`);
  const snapshot = res.data ?? res;
  if (snapshot.knowledge_base_id !== kbId || !snapshot.snapshot_id || !snapshot.state_as_of) {
