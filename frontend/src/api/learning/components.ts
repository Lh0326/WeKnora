...
export interface ComponentPlanInfo {policy_version:string;exact:boolean;expanded:number;candidates:number;utility:number;upper_bound:number;check_budget:number;notice?:string}
...
export async function getComponents(kb:string,minutes=15,goal=''):Promise<ComponentView>{const r:any=await get(`/api/v1/learning/kb/${kb}/components?minutes=${minutes}&goal=${encodeURIComponent(goal)}`);return r.data??r}
