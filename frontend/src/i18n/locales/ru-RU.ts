export default {
  ...
  error: {
    ...
    streamInterrupted: 'Предыдущий ответ был прерван (перезапуск сервиса или истёкший поток). Отправьте вопрос ещё раз',
    initialization: {
  ...
  knowledgeEditor: {
    learningTab: {
      ...
      title: 'Охват и навигация в сети знаний',
      litOf: 'Охвачено узлов: {lit} из {total}',
      noNodes: 'Сеть знаний ещё не создана — сначала постройте страницы в графе',
      goGraph: 'Открыть граф',
      lowConfidenceTip: 'Менее 3 фактов — уровень ориентировочный',
      tlFilteredEmpty: 'Среди загруженных записей нет совпадений',
      tlLoadMore: 'Загрузить ещё',
      constellationMore: 'Ещё {n} узлов с слабым сигналом не показаны',
      unitsTitle: 'По разделам',
      rootUnit: 'Корень',
      masteredOf: '{n} подтверждено',
      practiceN: 'Проверка знаний · {n} вопр.',
      levelKeep: 'Текущий уровень: {level} · эффективная знакомость {p}%',
      levelUpdate: 'Знакомость: {from} → {to}',
      optedOutNote: 'Сбор отключён: ответ не записывается, знакомость не меняется',
      collectOnNote: 'Сбор отключён: вопросы и проверки знаний больше не создают события обучения',
      profileSummary: '{nodes} узлов · {events} событий обучения · последняя активность {last}',
      tierListEmpty: 'В этом уровне пока нет узлов',
      tlFilterAll: 'Все',
      tlFilterCite: 'Вопросы',
      tlFilterCross: 'Повторы',
      tlFilterReAsk: 'Переспросы',
      tlFilterQuiz: 'Проверки',
      tlToday: 'Сегодня',
      tlYesterday: 'Вчера',
      reAskHint: 'Эти знания могут быть усвоены не до конца: прочтите страницу Wiki или пройдите проверку знаний, чтобы убедиться',
      deleteAlsoOptOut: 'Также остановить сбор (после удаления новые данные не собираются)',
      badgeEntity: 'Сущность',
      timelineEnd: 'Конец · всего {n} записей',
      badgeConcept: 'Концепция',
      tierUnseen: 'Не охвачено',
      tierFaded: "Угасло",
      explainTitle: "Почему такой уровень знакомости?",
      explainPEff: "Эффективная знакомость {p}% (с учётом угасания)",
      explainThresholds: "Охват при ≥30%, знакомо ≥55%, подтверждено ≥80%",
      explainEvidence: "Признаки: положительных {pos} · отрицательных {neg}",
      explainFaded: "Эти знания уже были затронуты, но непройденные проверки и повторные переспросы снизили знакомость, и со временем признаки угасли — узел вернулся в неохваченные; одна пройденная проверка быстро вернёт его",
      explainImprove: "Как поднять: пройденная проверка +2.2 · чтение на следующий день +0.5 · возврат из другого вопроса +1.6",
      tierTouched: 'Затронуто',
      tierFamiliar: 'Знакомо',
      tierMastered: 'Подтверждено',
      tierHintUnseen: 'Пока нет признаков изучения',
      tierHintTouched: 'Эффективная знакомость ≥ 30% (считается охваченным)',
      tierHintFamiliar: 'Эффективная знакомость ≥ 55% плюс пройденная проверка',
      tierHintMastered: 'Эффективная знакомость ≥ 80% плюс 2 разных задания проверки, пройденных в разные дни',
      howComputedTitle: 'Как считается прогресс',
      howComputedSignals: 'Цитирование в ответах, чтение страниц и проверки знаний накапливают признаки знакомости: цитирование +1.0, чтение страницы +0.5 (каждое открытие попадает в траекторию обучения; балл — раз в 48ч), повторный заход +1.6, пройденная проверка +2.2, непройденная -1.5, повторный вопрос -0.8, тематический сигнал +0.5.',
      howComputedGate: 'Для повышения нужны прямые подтверждения: чтение и цитирование дают не выше «Затронуто». Самостоятельный верный ответ открывает «Знакомо». Верные ответы на минимум два разных задания и успешные независимые проверки с интервалом ≥48 ч открывают «Подтверждено». Повтор того же задания в течение 48 ч после показа ответа даёт объяснение, но не добавляет свидетельств владения материалом.',
      howComputedDecay: 'Признаки со временем затухают: без закрепления эффективная знакомость постепенно снижается.',
      howComputedTiers: 'Уровни по эффективной знакомости: Затронуто ≥ 30% (охвачено), Знакомо ≥ 55%, Подтверждено ≥ 80%.',
      evidenceRefs: 'На основе {n} фрагментов источника',
      recommendTitle: 'Рекомендации по охвату',
      rsAffinity: 'Смежное',
      rsFrontier: 'Дальше',
      rsFoundation: 'С начала',
      rsContinue: 'Продолжить',
      rsConsolidate: 'Закрепить',
      rsReview: 'Давно',
      rsRemedial: 'Ещё раз',
      rsStruggling: 'Взглянуть снова',
      rsStuck: 'Сначала база',
      rsBypass: 'В обход',
      rsExplore: 'Разнообразие',
      rsSelfVerify: 'Ждём вас',
      recommendEmpty: 'Рекомендаций нет',
      zoneStartHere: 'Начните здесь',
      zoneCountTip: 'Освещено {lit} из {total} («уже освоено, убрать» считается освещённым)',
      zoneThenNote: 'на смену ① после завершения',
      zoneActionsBtn: 'Самооценка / освоено',
      zoneHintUnseen: 'Откройте и прочитайте, чтобы зажечь; после чтения пункт остаётся на ①, пока проверка не поднимет уровень',
      zoneHintTouched: 'Уже прочитано — пройдите проверку на странице, чтобы достичь «Знакомо»',
      zoneHintFamiliar: 'Хорошо идёт — пройдите 2 разные проверки в другой день, чтобы получить «Проверено»',
      zoneHintFaded: 'Раньше изучали, но позабылось; быстрая проверка вернёт',
      zoneQuizAvail: 'Доступно вопросов: {n}',
      objProgressVerified: 'Проверено {v}/{t} целей обучения',
      objProgressConflicting: ', {n} с конфликтом (требуется перепроверка)',
      objProgressStale: ', {n} устаревших (контент обновлён)',
      objProgressPartial: ', {n} частично (прошла одна группа)',
      objProgressUntested: ', {n} не тестировались',
      objProgressContact: 'Затронуто {c}/{n} узлов (контакт ≠ проверка)',
      objProgressSelfReport: '{n} самооценок (ожидают проверки)',
      pathTitle: 'Маршрут обучения',
      pathEmpty: 'Пока нет маршрута — задайте цель обучения',
      pathMinutes: 'мин',
      pathRequires: 'Пререквизит',
      pathActOverview: 'Обзор',
      pathActRead: 'Чтение',
      pathActBridge: 'Мост',
      pathActPractice: 'Практика',
      pathActVerify: 'Проверка',
      pathActReview: 'Повтор',
      pathWhyFirstContact: 'Первый контакт — прочитайте для базового понимания',
      pathWhyPractice: 'Контакт есть, проверки нет — потренируйтесь',
      pathWhyVerify: 'База есть — пройдите контракт проверки',
      pathWhyBridge: 'Сначала закройте пререквизит',
      pathWhyBridgeFail: 'Повторные неудачи — другой подход к пререквизиту',
      pathWhyReview: 'Проверено, но давно — повторите для подтверждения',
      pathWhySuccessor: 'Пререквизит проверен — следующий этап',
      pathWhyContinue: 'Продолжение вашей недавней темы',
      pathWhyFastTrack: 'Быстрый вызов — проверьте то, что уже знаете',
      pathWhyFallback: 'Начните с первого непосещённого узла',
      pathDegradeNoContent: 'Нет контента — загрузите документы и создайте Wiki',
      pathDegradeOrientation: 'Начинаем с первого незатронутого узла',
      objProgressGoalScope: 'Выбранные цели: проверено {v}/{t} (удаление целей не повышает покрытие библиотеки)',
      pathActionEnter: 'Начать',
      pathActionChallenge: 'Вызов',
      pathActionDisagree: 'Не согласен',
      nodeContactedYes: 'Затронуто',
      nodeContactedNo: 'Не затронуто',
      nodeSelfUp: 'Самооценка: знаю лучше',
      nodeSelfDown: 'Самооценка: знаю хуже',
      nodeLastVerified: 'Последняя проверка',
      nodeNextVerified: 'Далее: перейти к следующим',
      nodeNextConflicting: 'Далее: новый раунд проверки',
      nodeNextStale: 'Далее: перепроверить по новому',
      nodeNextPartial: 'Далее: вторая группа вопросов',
      nodeNextUnverified: 'Далее: чтение + практика + проверка',
      nodeNextRetired: 'Пропущено (можно восстановить)',
      nodeNextChallenge: 'Ожидает вызова',
      pathActionSwitchMaterial: 'Сменить материал',
      pathActionViewEvidence: 'Показать доказательства',
      pathActionExport: 'Экспорт профиля',
      pathActionDelete: 'Удалить профиль',
      zonePathCaption: 'Маршрут обучения (от простого к сложному; нажмите узел, чтобы учить)',
      zonePathLegend: 'Цветной значок справа = уровень и эффективная освоенность этого узла',
      zoneExpandPath: 'Развернуть маршрут ({n})',
      zoneCollapsePath: 'Свернуть маршрут',
      zoneCurrentMark: 'сейчас',
      reasonAffinity: 'Часто используемые вами документы касаются этих знаний',
      reasonFrontier: 'Следующий шаг на границе знаний',
      reasonConsolidate: 'Закрепите затронутую тему',
      reasonReview: 'Нужно закрепить: знакомость угасает',
      reasonRemedial: 'Нужна перепроверка: эти знания ранее не прошли проверку',
      reasonStruggling: 'Повторные переспросы: эти знания многократно переспрашивались',
      reasonStuck: 'Пререквизит долго не подтверждён — начните с него',
      reasonBypass: 'Цель заблокирована пререквизитом — начните со соседней',
      reasonExplore: 'Исследование слепой зоны',
      reasonSelfVerify: 'Самодекларация ждёт подтверждения: 1 новый верный ответ открывает «Знакомо», ещё один на следующий день — «Подтверждено»',
      reasonFoundation: 'Начальная точка: открывает исходные материалы — первая остановка при старте с нуля',
      reasonContinue: 'Продолжение: следующий после изученного',
      zoneIntro: 'Следуйте за меткой «сейчас»: она движется по маршруту по мере закрепления; разверните, чтобы видеть весь путь. Справа сверху = освещено/всего («уже освоено, убрать» тоже считается)',
      zoneDone: 'Модуль полностью освещён',
      zoneNextMark: 'Первый шаг модуля',
      constellationHint3d: 'Внутренние орбиты = проверено · секторы = модули · наведите для связей · колесо — масштаб · перетаскивание вращает вид · двойной клик — сброс',
      constellationDustHint: 'Непройденные узлы: {n} — нажмите, чтобы развернуть',
      whyFirstStop: 'Первая остановка — здесь складывается общая картина',
      whyMaterialStart: 'Позиция №{rank} в материале — начальный узел «{section}»',
      whyContinuesPrereq: 'Продолжает только что изученное «{ref}» — его естественный следующий шаг',
      whyContinuesPrev: 'Идёт после предыдущей рекомендации «{ref}» — шаг за шагом',
      whySameSection: 'Та же глава «{section}», что и «{ref}» — учитесь сразу следом',
      whyChapterOf: 'Относится к «{label}»',
      whyInDoc: 'Из «{label}»',
      skipAction: 'Уже освоено — скрыть из очереди',
      skipNote: '«Уже освоено — скрыть из очереди»: скрытые узлы исчезают из «Что изучить дальше» и дайджеста забывания; вернуть — из списка уровней',
      skipDone: 'Узел убран из очереди рекомендаций',
      skipRestored: 'Возвращено в рекомендации',
      skipFailed: 'Не удалось выполнить операцию, попробуйте снова',
      skippedTag: 'Пропущено',
      skippedBadge: 'Пропущено: самостоятельно объявлено освоенным (узел можно открыть и изучать; вернуть в рекомендации — из списка уровней)',
      restoreSkip: 'Вернуть',
      selfVerifyGoVerify: 'Пройти проверку',
      selfVerifyQuizPending: 'Вопросы проверки готовятся — сначала прочитайте страницу; когда вопросы появятся, 1 верный ответ откроет «Знакомо»',
      recommendOrderIntro: 'От простого к сложному по материалу: начните с №1 и идите по порядку; тег каталога — только принадлежность',
      recommendStartHere: 'Начните здесь',
      explainDocRank: 'Позиция в материале: {n} (1 = начало; рекомендации следуют этому порядку)',
      explainSource: 'Источник: «{section}» — «{doc}»',
      selfAssessUpDoneNoQuiz: 'Балл поднят; когда вопросы проверки появятся, 1 верный ответ откроет «Знакомо»',
      tierListLoadFailed: 'Не удалось загрузить список',
      changesIdle: 'без повторения {d} дн.',
      changesNext: 'понижение через ~{n} дн.',
      openPage: 'Открыть',
      practice: 'Проверка знаний',
      quizTitle: 'Проверка знаний: {slug}',
      quizEmpty: 'Для этих знаний пока нет проверочных заданий',
      submitAnswer: 'Ответить',
      nextQuestion: 'Следующий вопрос',
      evidenceSource: 'Источник',
      nextReviewDays: 'Прогноз свежести: эти знания сохранятся ещё около {n} дн.',
      reviewNow: 'Прогноз свежести: скорее проверьте эти знания',
      kbdHint: 'A–D выбор · E не уверен · Enter отправить',
      unsureOption: 'Не уверен — сначала показать разбор',
      answerUnsure: 'Вы выбрали «не уверен»',
      unsureNote: 'Ответ «не уверен» не даёт и не отнимает баллов: прочитайте разбор ниже и ответите снова в другой день.',
      pEffMove: 'Эффективная знакомость {from}% → {to}%',
      todayAnswers: 'Сегодня: проверок {n} · пройдено {m}',
      todayLit: 'Сегодня охвачено новых узлов: {n}',
      todayEvents: 'Сегодня: {n} событий',
      todayEventsNote: 'События обучения за сегодня: чтение страниц, цитаты в ответах, проверки знаний и самодекларации',
      todayCardTitle: 'Закрепить сегодня',
      todoDemoted: 'Понижен · закрепить',
      todoDue: 'Скоро понижение',
      reasonRemedialShort: 'Перепроверка',
      reasonStrugglingShort: 'Переспрашиваете',
      reasonStuckShort: 'База не подтверждена',
      hintFirstTouch: 'Откройте страницу или задайте вопрос, чтобы охватить',
      hintQuizUnlockFamiliar: 'Пройденное новое задание проверки откроет «Знакомо»',
      hintQuizUnlockMastered: 'Ещё одно новое задание в другой день откроет «Подтверждено»',
      hintGrowMastery: 'Продолжайте проходить проверки, чтобы растить знакомость',
      hintKeepReviewing: 'Следуйте прогнозу свежести, чтобы сохранить уровень',
      evUnsure: 'Признание «не уверен»',
      hoverOpenNode: 'Открыть эту страницу знаний',
      tierNodeTip: 'Эффективная знакомость {p}% · признаков: {n} · нажмите, чтобы открыть',
      tlNodeChipTip: 'Показать {n} событий этого узла',
      evidenceSourceTip: 'Открыть документ-источник этого проверочного задания',
      constellationTitle: 'Созвездие знаний',
      constellationLit: 'охвачено',
      constellationHint: 'Внутренние кольца = глубже знакомость · крупнее точка = больше признаков · пунктир = мало данных · янтарный = угасло · мерцание = касались за последние 48 ч · щёлкните узел, чтобы открыть',
      constellationHoverP: 'Эффективная знакомость {p}%',
      constellationHoverN: 'Признаков: {n}',
      quizLoadFailed: 'Не удалось загрузить вопросы, попробуйте позже',
      loadError: 'Не удалось загрузить данные обучения — проверьте сервис и повторите',
      retry: 'Повторить',
      collectKeptNote: 'Пауза останавливает только новые записи; существующие {n} записей сохраняются и могут быть экспортированы или удалены в любой момент',
      deleteIrreversible: 'Я понимаю, что это действие нельзя отменить',
      ...
      deleteDone: 'Профиль удалён',
      deleteFailed: 'Не удалось удалить профиль',
      toggleFailed: 'Настройка не удалась, отменено',
      answerCorrect: 'Верно',
      answerWrong: 'Неверно',
      timelineTitle: 'Траектория обучения',
      selfAssessSection: 'Эта оценка знакомости кажется неточной?',
      selfAssessUp: 'Я лучше знаком с этими знаниями',
      selfAssessDown: 'Я подзабыл эти знания',
      selfAssessUpDone: 'Балл поднят до зоны «подтверждено»; 1 пройденная проверка открывает «Знакомо», ещё одна на следующий день официально открывает «Подтверждено»',
      selfAssessDownDone: 'Знакомость скорректирована по выбранной причине',
      selfAssessFailed: 'Не удалось отправить самодекларацию, повторите',
      selfAssessDialogTitle: 'Какая часть вам менее знакома?',
      selfAssessSubmit: 'Отправить',
      saReasonAll: 'Совсем не владею (ранее промахнулся)',
      saReasonAllHint: 'Сбросить знакомость этих знаний до нуля и выстроить охват заново',
      saReasonDocGap: 'Документы не покрывают эту часть',
      saReasonDocGapHint: 'Понизить на одну ступень и записать пробел в документации',
      saReasonDocUpdated: 'В документах новый материал',
      saReasonDocUpdatedHint: 'Понизить на одну ступень и записать устаревание знания',
      saReasonQuizEasy: 'Тест был слишком лёгким',
      saReasonQuizEasyHint: 'Понизить на одну ступень и записать проблему валидности теста',
      saBadgeUp: 'Самодекларация: выше (ждёт проверки — 1 новый верный ответ открывает «Знакомо», ещё один на следующий день — «Подтверждено»)',
      saBadgeDownAll: 'Самодекларация: сброс',
      saBadgeDownDocGap: 'Самодекларация: ниже (пробел в доках)',
      saBadgeDownDocUpdated: 'Самодекларация: ниже (доки обновлены)',
      saBadgeDownQuizEasy: 'Самодекларация: ниже (тест лёгкий)',
      evSelfAssessUp: 'Самодекларация вверх',
      evSelfAssessDownAll: 'Самодекларация: сброс',
      evSelfAssessDownDocGap: 'Самодекларация вниз: пробел в доках',
      evSelfAssessDownDocUpdated: 'Самодекларация вниз: доки обновлены',
      evSelfAssessDownQuizEasy: 'Самодекларация вниз: валидность проверки',
      timelineTodayCount: 'Сегодня: записей обучения — {n}',
      timelineEmpty: 'Событий обучения пока нет',
      evCite: 'Цитата',
      evCross: 'Повтор',
      evReAsk: 'Переспрос',
      evTopic: 'Сигнал темы',
      evDeepRead: 'Глубокое чтение',
      evRead: 'Чтение страницы',

      evAgentRead: 'Запрос помощника',
      evQuizOk: 'Проверка пройдена',
      evQuizBad: 'Проверка не пройдена',
      evBackfill: 'История',
      profileScopeNote: 'Эти действия охватывают ваш профиль обучения по ВСЕМ базам знаний',
      tlMoreNodes: 'Показать все {n} узлов',
      tlFewerNodes: 'Свернуть',
      profileTitle: 'Управление профилем',
      stopCollect: 'Не собирать мои данные о знакомости',
      export: 'Экспорт профиля',
      delete: 'Удалить профиль',
      deleteConfirmTitle: 'Удалить профиль обучения',
      deleteConfirmBody: 'Будут удалены все ваши события обучения, состояния знакомости, привязки тем и попытки проверок (общий банк вопросов сохраняется). Действие необратимо.',
      // ---- здоровье знаний (владелец/админ) и план покрытия по ролям ----
      viewPersonal: 'Личный вид',
      viewHealth: 'Здоровье знаний',
      healthTitle: 'Здоровье активов знаний',
      healthLoadFailed: 'Не удалось загрузить данные о здоровье',
      healthSummaryNodes: 'Узлов знаний: {total}',
      healthSummaryCovered: 'Охвачено: {n}',
      healthSummarySubjects: 'Пользователей с записями обучения: {n}',
      healthFoldersTitle: 'Охват по папкам',
      healthFolderCovered: '{covered}/{total}',
      healthExpertsTitle: 'Распределение экспертов (по числу знакомых)',
      healthExpertCount: 'знакомы: {n}',
      healthRisksTitle: 'Риски знаний',
      riskSinglePoint: 'Единая точка знания: знаком только 1 человек',
      riskStaleDoc: 'Документальные знания забыты: изучались ранее, но знакомость всех снизилась',
      healthMaintenanceTitle: 'Входящие задачи управления',
      maintPendingVerify: 'Самодекларации, ожидающие проверки',
      maintDownAll: 'Самодекларация: сброшена',
      maintDocGap: 'Пробел в охвате документами',
      maintDocUpdated: 'Знания устарели (документ обновлён)',
      maintQuizEasy: 'Сомнительная валидность проверочных вопросов (слишком лёгкие)',
      maintLatestAt: 'Последнее: {t}',
      maintEmpty: 'Нет меток обслуживания',
      risksEmpty: 'Нет рисков',
      expertsEmpty: 'Нет записей знакомости',
      rolePlanTitle: 'План покрытия по ролям',
      rolePlanIntro: 'Рекомендации по охвату, сгруппированные по папкам — сначала закройте неохваченные и требующие закрепления знания в своей зоне ответственности',
    },
    ...
    wikiBrowser: {
      ...
      mcSummary: 'Эффективная освоенность {p}% · событий: {n}',
      mcLastTouch: 'Последнее касание',
      mcOpenNode: 'Щёлкните узел, чтобы открыть страницу',
      mcQuizN: 'Доступно вопросов: {n}',
      mcLowConf: 'Недостаточно данных (<3) — уровень предварительный',
      masteryLegendNew: 'NEW = подсвечено в этой сессии',
      masteryNoRing: 'Без кольца · не покрыто',
      masteryLowConfBadge: 'Низкая уверенность (пунктир)',
      masteryUnseen: 'Позабыто (есть история)',
      masteryTouched: 'Затронуто',
      masteryFamiliar: 'Знакомо',
      masteryMastered: 'Подтверждено',
      ...
      assistantName: 'Помощник по знаниям',
      assistantNewChat: 'Новый чат',
      assistantTitle: 'Помощник WeKnora',
      assistantSceneWikiPage: 'Вы на странице: «{title}»',
      assistantSceneWiki: 'Вы в: вики-карте знаний',
      assistantSceneLearning: 'Вы во вкладке: обучение · созвездие',
      assistantSceneDocuments: 'Вы в: документах базы',
      assistantSceneKb: 'Вы в: этой базе знаний',
      assistantTooltip: 'Спросить помощника',
      assistantRetry: 'Не удалось создать сессию — нажмите, чтобы повторить',
