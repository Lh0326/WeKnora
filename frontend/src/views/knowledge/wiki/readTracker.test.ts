...
test('visible dwell resumes after hiding, ignores background time, and stops cleanly', () => {
  ...
  try {
    ...
    tracker.stop()
    const shortRecords:string[]=[]
    const short=createWikiReadTracker((slug,tier)=>shortRecords.push(`${slug}:${tier}`),()=>8_000)
    short.start();short.enter('short');tick(8_000)
    assert.deepEqual(shortRecords,['short:normal','short:deep'])
    short.restart();tick(7_999)
    assert.equal(shortRecords.filter(x=>x==='short:deep').length,1,'a correction must start a fresh dwell window')
    tick(1)
    assert.equal(shortRecords.filter(x=>x==='short:deep').length,2,'revisit feedback should work without closing the page')
    visibility(true);short.restart();tick(60_000)
    assert.equal(shortRecords.length,4,'hidden time after a correction must not count')
    short.stop();visibility(false)
    tracker.stop()
    visibility(false)
    tick(120_000)
