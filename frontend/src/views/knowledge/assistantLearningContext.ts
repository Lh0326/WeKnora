...
export function snapshotFields(snapshot:AssistantLearningSnapshot|null,slug:string|null,session?:LearningSessionSnapshot|null):Record<string,string>|null {
 ...
 if(session){
  ...
  fields.learning_scope=`与学习导航共用的本轮计划：${session.label}；内部推荐上限 ${req.time_budget_minutes??15} 分钟；深度 ${depth}；已临时跳过 ${req.excluded_slugs?.length||0} 个节点；${req.use_memory===false?'未启用':'已启用'}长期记忆参考。学习范围按模块选择，页面无需设置学习时长，不要说这是用户选择的时长。请依据这份计划回答下一步；不要自行替换为全库默认计划。`
 ...
 if(plan?.focus_slug)fields.learning_path_method=`本轮推进：${plan.focus_title||plan.focus_slug}；${plan.progression||''}。${(plan.limitations||[]).join(' ')} 这是知识页的阅读计划；未提供已审核能力目标时，不宣称已有应用练习或能力递进。`
