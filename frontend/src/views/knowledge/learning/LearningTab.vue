  ...
  <div class="learning-tab">
    ...
    <div v-else class="sky" :class="{ 'health-view': healthView }">
      ...
      <div class="sky-head">
        ...
        <!-- 视图切换（健康视图时在头部行右侧；个人视图的同一组按钮锚在
             星图区右上角——两个视图恒定"内容区右上"语义，不再左右漂移） -->
        <div v-if="canManage && healthView" class="view-toggle">
          <button class="vt-btn" :class="{ active: !healthView }" :aria-pressed="!healthView" @click="healthView = false">{{ $t('knowledgeEditor.learningTab.viewPersonal') }}</button>
          <button class="vt-btn" :class="{ active: healthView }" :aria-pressed="healthView" @click="healthView = true">{{ $t('knowledgeEditor.learningTab.viewHealth') }}</button>
        </div>
      </div>
      <template v-if="!healthView">
        <div class="sky-chart">
          <!-- 与健康视图头部右侧同一组按钮：个人视图锚在星图区右上角 -->
          <div v-if="canManage" class="view-toggle chart-anchored">
            <button class="vt-btn" :class="{ active: !healthView }" :aria-pressed="!healthView" @click="healthView = false">{{ $t('knowledgeEditor.learningTab.viewPersonal') }}</button>
            <button class="vt-btn" :class="{ active: healthView }" :aria-pressed="healthView" @click="healthView = true">{{ $t('knowledgeEditor.learningTab.viewHealth') }}</button>
          </div>
          <!-- 零节点空态：Wiki 库还没有 entity/concept 页面时给出明确去向，
               而不是渲染 0/0 空环让人无从下手 -->
          ...
          <div v-else class="empty-nodes">
            ...
            <div class="empty-nodes-text">{{ $t('knowledgeEditor.learningTab.noNodes') }}</div>
            <t-button size="small" variant="outline" @click="goGraph">{{ $t('knowledgeEditor.learningTab.goGraph') }}</t-button>
          </div>
        </div>
        <div class="sky-side">
        <div class="side-scroll">
          ...
          <!-- 下一步看什么：窄栏紧凑行。从浅入深——一条按资料顺序展开的线性路径
               （1=第一站，依次往下），目录只作为行内归属标签，不再分组成并列模块，
               避免"多个起点无从下手"的歧义。 -->
          <div class="side-block">
            ...
            <div v-else class="zone-card-list">
              <div v-for="zone in zoneList" :key="zone.folder_id || 'root'" class="zone-card"
                ...
                <template v-if="zone.next">
                  <!-- 极简卡：①短理由+标题（整行可点，悬停看完整理由说明），
                       ②灰字下一项；按钮行与叙事长句已删——答题入口在节点
                       自己的 wiki 页面（学习发生的地方），比卡片上的按钮自然。 -->
                  <div class="zone-next" role="button" tabindex="0"
                    ...
                    <span class="rec-reason-tag" :style="reasonStyle(zone.next.reason)">{{ reasonShort(zone.next.reason) }}</span>
                  ...
                  <button class="zone-esa-toggle" :title="$t('knowledgeEditor.learningTab.selfAssessSection')"
                    ...
                    <svg viewBox="0 0 16 3" width="14" height="4" aria-hidden="true">
                      <circle cx="2" cy="1.5" r="1.2" fill="currentColor" />
                      <circle cx="8" cy="1.5" r="1.2" fill="currentColor" />
                      <circle cx="14" cy="1.5" r="1.2" fill="currentColor" />
                    </svg>
                  </button>
...
function reasonStyle(reason: string): { background: string; color: string } {
  const map: Record<string, { background: string; color: string }> = {
    ...
    // 入门起点（文档开篇位置）：品牌绿更实一分，与 frontier 区分
    foundation: { background: 'rgba(3, 134, 38, 0.14)', color: '#026a2e' },
    // 顺承（承接刚学/上一个推荐）：品牌绿系弱一档，与入门起点区分
    continue: { background: 'rgba(4, 155, 56, 0.10)', color: '#047a3d' },
...
// 而不是让 0/0 空环自己解释自己。
const hasNodes = computed(() => (progress.value?.total_nodes ?? 0) > 0)
function goGraph() {
...
/* 个人视图的切换按钮悬浮于星图区右上角（与健康视图头部右侧同一锚点语义） */
.sky-chart .view-toggle.chart-anchored { position: absolute; top: 8px; right: 8px; z-index: 2; }
...
.zone-esa-toggle {
  ...
  color: var(--td-text-color-placeholder, #bbb); cursor: pointer;
}
.zone-esa-toggle:hover { background: var(--td-brand-color-light, rgba(7, 192, 95, 0.1)); color: #049b38; }
