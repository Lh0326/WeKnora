...
const identity=computed(()=>JSON.stringify([auth.user?.id,auth.currentTenantId]))
const requestScope=createRequestScope(()=>identity.value)
let readingCurrent=requestScope.capture()
...
const tracker=createWikiReadTracker((slug,tier)=>{if(!readingCurrent())return;const current=readingCurrent,kb=readingKb;notifyReadStatus(kb,slug,'saving',tier);recordWikiRead(kb,slug,tier).then(result=>{if(!current())return;if(result.data?.recorded===false){notifyReadStatus(kb,slug,'disabled',tier);return}notifyReadStatus(kb,slug,'saved',tier);notifyLearningUpdated(kb,slug)}).catch(()=>{if(current())notifyReadStatus(kb,slug,'error',tier)})},()=>readingThresholdMs(page.value?.content||''))
...
async function load(){const gen=++generation;tracker.settle();page.value=null;error.value='';if(!props.visible||!props.slug)return;loading.value=true;const kb=props.kbId,slug=props.slug;try{const result:any=await getWikiPage(kb,slug);if(gen!==generation)return;const loaded:WikiPage=result.data??result;if(!loaded?.slug||!['entity','concept'].includes(loaded.page_type))throw Error('not a learning node');page.value=loaded;readingKb=kb;readingCurrent=requestScope.capture();tracker.enter(loaded.slug);publishPage()}catch{if(gen===generation)error.value='知识内容加载失败，请重试或在 Wiki 中查看。'}finally{if(gen===generation)loading.value=false}}
...
watch(identity,()=>{requestScope.invalidate();generation++;tracker.stop();page.value=null},{flush:'sync'})
onMounted(()=>{tracker.start();window.addEventListener(LEARNING_UPDATED,feedbackUpdated)});onUnmounted(()=>{generation++;tracker.stop();requestScope.dispose();window.removeEventListener(LEARNING_UPDATED,feedbackUpdated)});onActivated(()=>{tracker.start();if(props.visible&&page.value){tracker.enter(page.value.slug);publishPage()}});onDeactivated(()=>{generation++;tracker.stop()})
</script>
<style scoped>
