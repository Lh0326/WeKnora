<template>
 <section class="objective-workspace" aria-label="目标与学习路径">
  ...
  <div class="session-settings"><label>学习范围<select v-model="moduleId" :disabled="busy" @change="chooseModule(moduleId)"><option :value="null">全部模块</option><option v-for="group in modules" :key="group.id" :value="group.id">{{ group.name }} · {{ group.total }} 个知识点</option></select></label><button class="text-button" :disabled="busy" @click="openSettings">学习偏好</button></div>
  ...
  <div class="workspace-links">
   ...
   <button class="text-button" @click="openSettings">学习偏好</button>
  </div>
  <t-drawer v-model:visible="settingsOpen" header="学习偏好" size="480px" :footer="false" @close="cancelSettings">
   <button @click="cancelSettings">返回学习</button>
   <p class="hint">结合阅读进度和前置关系安排下一步，无需设置学习时长。没有题库也能通过阅读自动推进；反馈用于调整推荐。</p>
   <p class="hint">学习范围在导航中按模块选择。学习深度、可选验证目标与记忆偏好会随账号保存；已有基础选项与临时跳过仅影响本轮。</p>
   ...
   <details v-if="published.length"><summary>验证目标（可选{{ selected.length ? `，已选 ${selected.length} 个` : '' }}）</summary><p class="hint">这是希望检验的具体能力，不是学习范围。不选择时，根据学习深度自动安排；缺少应用材料或任务时不会虚构练习。</p><label v-for="o in published" :key="o.objective_id" class="goal"><input v-model="selected" type="checkbox" :value="o.objective_id" />{{ o.title }}</label><button v-if="selected.length" @click="selected=[]">清空所选验证目标</button></details>
   <p v-if="!published.length" class="hint">本库暂无已审核验证目标，将按知识点内容与模块顺序规划阅读。</p>
   <p v-if="error" role="alert">{{ error }}</p>
   <button :disabled="busy" @click="applySettings">保存偏好</button>
  </t-drawer>
...
import {notifyLearningUpdated,nodeStateColors} from './learningEvents'
...
const requestScope=createRequestScope(()=>scopeKey.value)
...
const notice=ref('')
const depth=ref('aware'),fastTrack=ref(false),selected=ref<string[]>([]),busy=ref(false),error=ref('')
const planningMinutes=15
...
async function refresh(persist=false){
 ...
 const applied=settingsOpen.value&&!persist?savedSettings:{depth:depth.value,fastTrack:fastTrack.value,useMemory:useMemory.value,selected:selected.value}
 const request={goal_slugs:learningScopeSlugs(view.value?.nodes||[],selectedModule),excluded_slugs:[...excluded.value],goal_objectives:[...applied.selected],depth:applied.depth,time_budget_minutes:planningMinutes,fast_track:applied.fastTrack,use_memory:applied.useMemory}
...
async function markKnown(slug:string){if(actionBusy.value)return;const kb=props.kbId,current=requestScope.capture();actionBusy.value=true;try{await setNodeState(kb,slug,'known');if(!current())return;await refresh();if(!current())return;notice.value='已保存纠偏，熟悉程度与后续建议已重新估计。';notifyLearningUpdated(kb,slug)}catch{if(current())error.value='保存失败，纠偏尚未生效，请重试。'}finally{if(current())actionBusy.value=false}}
...
onUnmounted(()=>{requestScope.dispose();++generation;++taskGeneration;clearTimeout(recallTimer);preferences.clear();document.removeEventListener('visibilitychange',visibleAgain)})
onActivated(()=>{const resume=!active;active=true;if(resume)refresh()});onDeactivated(()=>{active=false;clearTimeout(recallTimer)})
let savedSettings = {depth:'aware',fastTrack:false,useMemory:true,selected:[] as string[]}
function openSettings(){savedSettings={depth:depth.value,fastTrack:fastTrack.value,useMemory:useMemory.value,selected:[...selected.value]};settingsOpen.value=true}
function cancelSettings(){depth.value=savedSettings.depth;fastTrack.value=savedSettings.fastTrack;useMemory.value=savedSettings.useMemory;selected.value=[...savedSettings.selected];settingsOpen.value=false}
...
async function gradeTask(){
 ...
 try{const r=await submitTask(kb,task.value.id,answers.value,helped.value?'assistant_helped':task.value.assistance_mode);if(gen!==taskGeneration)return;verdict.value=r;notifyLearningUpdated(kb,taskTarget.value.slug);await refresh()}catch{if(gen===taskGeneration)taskError.value='未收到检查结果，答案已保留。可先返回查看进度，再决定是否重试。'}finally{if(gen===taskGeneration)taskBusy.value=false}
}
watch(scopeKey,()=>{requestScope.invalidate();++generation;++taskGeneration;actionBusy.value=false;moduleId.value=null;excluded.value=[];taskBusy.value=false;taskLoading.value=false;taskError.value='';selected.value=[];depth.value='aware';fastTrack.value=false;useMemory.value=true;settingsOpen.value=false;view.value=null;progress.value=null;plan.value=null;emit('view',{projection_version:'unavailable',entries:[]});emit('plan',null);taskOpen.value=false;evidenceOpen.value=false;preferences.setScope(props.kbId);preferenceError.value='';preferenceNotice.value='';restorePreferences()},{immediate:true,flush:'sync'})
async function restorePreferences(){
 const kb=props.kbId,gen=++generation;busy.value=true;preferenceError.value=''
 learningSession?.begin(kb,{},'正在恢复学习范围')
 try{const p=await preferences.load();if(gen!==generation||!p)return;moduleId.value=p.limit_to_folder?p.folder_id:null;depth.value=p.depth;useMemory.value=p.use_memory;selected.value=[...p.goal_objectives];excluded.value=[];fastTrack.value=false;preferenceNotice.value=p.revision?'已恢复上次保存的学习范围。':''}
...
.session-settings label{display:flex;align-items:center;gap:10px}.session-settings select{flex:1;min-width:0;width:100%}.controls summary{cursor:pointer}
