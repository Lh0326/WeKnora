...
test('assistant uses exactly the five-minute module plan, including exclusions and the displayed explanation', () => {
  ...
  const scopedPlan = {...plan('检索配置'),focus_slug:'检索配置',focus_title:'检索配置',progression:'先阅读必要准备，再推进当前目标。',limitations:['未配置应用任务，不推断应用能力。']}
  session.begin('kb', { goal_slugs: ['检索'], excluded_slugs: ['已跳过'], time_budget_minutes: 5, depth: 'operate' }, '检索模块')!.complete(view, scopedPlan)
  const fields = snapshotFields(defaultSnapshot, null, session.snapshot.value)!
  assert.match(fields.learning_scope!, /检索模块；内部推荐上限 5 分钟；深度 完成操作；已临时跳过 1/)
  assert.match(fields.learning_scope!, /不要说这是用户选择的时长/)
  assert.match(fields.next_recommended!, /检索配置.*继续你选择的检索模块/)
  assert.match(fields.learning_path_method!, /检索配置.*先阅读必要准备.*未配置应用任务/)
