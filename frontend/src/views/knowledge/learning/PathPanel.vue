<template>
 <section class="path-panel" aria-label="下一步学习">
  <div class="path-heading"><h3>学习路径</h3><span v-if="plan?.steps.length && !busy">{{ plan.steps.length }} 步 · 约 {{ totalMinutes }} 分钟</span></div>
  <template v-if="current && !busy">
   <p v-if="plan?.focus_title && plan.focus_slug!==current.slug" class="reason">这条路径要达到的目标：<strong>{{ plan.focus_title }}</strong></p>
  ...
  <p v-if="plan?.progression && !busy" class="reason">{{ plan.progression }}</p>
  <details v-if="plan && !busy" class="path-explanation"><summary>为什么采用这个顺序</summary><p>先处理困难与到期复习，再尽量接续近期学习的模块，围绕当前目标安排必要前置。每次学习后重新计算。</p><p>只有明确记录的先修关系才约束先后。主题分类、相似内容和原文位置不证明知识层级；没有应用任务时，排序只能帮助阅读。</p><p v-for="limitation in plan.limitations||[]" :key="limitation">{{ limitation }}</p></details>
  <p v-if="plan?.degrade && !busy" class="path-note" role="status">{{ degradeLabel(plan.degrade) }}</p>
...
.path-explanation{font-size:12px;line-height:1.65;color:var(--td-text-color-secondary)}.path-explanation summary{cursor:pointer}.path-explanation p{margin-top:8px}
