...
async function fixture(agentMode=false, onError?: (message:string)=>void, onLearningUpdated?: (ids:string[])=>void){
 const messages=reactive<ChatMessage[]>([])
 let handler!:ReturnType<typeof useChatStreamHandler>
 const app=createSSRApp({setup(){handler=useChatStreamHandler({messagesList:messages,loading:ref(false),isReplying:ref(false),currentAssistantMessageId:ref(''),fullContent:ref(''),isAgentStreamSession:()=>agentMode,scrollToBottom:()=>{},preserveIncompleteStreamReactive:true,onError,onLearningUpdated});return()=>null}})
...
test('a quota failure before any answer remains visible with compact toast and retained details',async()=>{
 const notices:string[]=[]
 const {messages,handler}=await fixture(false,message=>notices.push(message))
 const raw='API request failed with status 403: {"code":"AllocationQuota.FreeTierOnly","message":"Free quota exhausted.","request_id":"provider-request"}'
 handler.processStreamChunk({response_type:'agent_query',id:'request',assistant_message_id:'assistant'})
 handler.processStreamChunk({response_type:'error',id:'request',content:raw})
 const row=messages[0]!
 assert.equal(row.is_completed,true)
 assert.equal(handler.shouldRenderAssistantMessage(row),true)
 assert.equal((row.chatError as {raw:string}).raw,raw)
 assert.equal(row.content,raw)
 assert.equal(notices.length,1)
 assert.match(notices[0]!,/free quota/i)
 assert.doesNotMatch(notices[0]!,/request_id|\{/)
})

test('an error payload can retain explicit failing-model context without using requested model guesses',async()=>{
 const {messages,handler}=await fixture()
 handler.processStreamChunk({response_type:'agent_query',id:'request'})
 handler.processStreamChunk({response_type:'error',id:'request',data:{error:{code:'AccessDenied',message:'HTTP 403'},error_context:{model_name:'actual-model',provider:'actual-provider',model_type:'rerank'}}})
 const failure=messages[0]!.chatError as {raw:string;modelName:string;modelType:string}
 assert.match(failure.raw,/AccessDenied/)
 assert.equal(failure.modelName,'actual-model')
 assert.equal(failure.modelType,'rerank')
 assert.equal(handler.shouldRenderAssistantMessage(messages[0]!),true)
})

test('learning invalidation waits for the committed completion boundary, not answer tokens',async()=>{
 const seen:string[][]=[]
 const {handler}=await fixture(true,undefined,ids=>seen.push(ids))
 handler.processStreamChunk({response_type:'agent_query',id:'request',assistant_message_id:'answer'})
 handler.processStreamChunk({response_type:'answer',id:'request',content:'answer',done:true})
 assert.deepEqual(seen,[])
 handler.processStreamChunk({response_type:'complete',id:'request',done:true,data:{learning_kb_ids:['kb','kb','',null,23]}})
