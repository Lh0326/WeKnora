...
const PathPolicyVersion = "path-plan-v9-focus"
...
// PathPlan is the bounded reading/verification path plus policy metadata.
...
type planInput struct {
	// Internal one-focus recursion guard; no persisted preference or new API.
	FocusResolved      bool
	SkipOptionalCheck  bool
	...
	// DocOrder: source-position fallback rank, not a pedagogy/difficulty rank.
...
func planShortPath(in planInput) PathPlan {
	if in.IncludeExploration && in.GoalsResolved && in.Estimates != nil && len(in.UserGoals) == 0 && !in.FastTrack && !in.FocusResolved {
		return planFocusedReading(in)
	}
	...
	if len(goals) == 0 && len(plan.Steps) == 0 && in.Estimates != nil && !in.SkipOptionalCheck {
