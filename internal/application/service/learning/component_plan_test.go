...
func TestComponentDifficultWithoutNewChecksCanContinueWithRecall(t *testing.T) {
	for _, noChecks := range []bool{true, false} {
		t.Run(fmt.Sprintf("no_checks_%v", noChecks), func(t *testing.T) {
			rows, _ := componentFixture(t)
			c := rows[0]
			if noChecks {
				c.Definition.Checks = nil
			}
			now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
			events := []types.LearningEvent{
				componentEvent(c, "difficulty", "difficult", now.Add(-time.Hour)),
				componentEvent(c, "read", "read", now.Add(-time.Minute)),
			}
			if !noChecks {
				seen := componentEvent(c, "old-check", "check", now.Add(-2*time.Hour))
				events = append(events, seen)
			}
			before := projectComponent(c, events, now)
			entry := interfaces.ComponentEntry{ID: c.ID, Available: true, Material: c.Definition, State: before}
			steps, used, _ := planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", now)
			require.Len(t, steps, 1, "difficulty must not lead to a dead end when no new check is available")
			require.Equal(t, "recall", steps[0].Action)
			require.Equal(t, 1, used)
			require.Contains(t, steps[0].Reason, "没有新检查")

			recall := componentEvent(c, "real-recall", "recall", now)
			var fact componentFact
			require.NoError(t, json.Unmarshal(recall.ReviewData, &fact))
			fact.Rating = 3
			recall.ReviewData, _ = json.Marshal(fact)
			events = append(events, recall)
			after := projectComponent(c, events, now)
			require.Equal(t, now, after.LastRecallAt)
			require.NotNil(t, after.DueAt)
			require.True(t, after.DueAt.After(now))
			require.Equal(t, "review", after.Level, "recall self-report must not turn difficulty into measured mastery")
			require.Equal(t, before.Familiarity, after.Familiarity)
			require.Equal(t, before.Checks, after.Checks)
			entry.State = after
			steps, _, _ = planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", now)
			require.Empty(t, steps, "a completed recall must not be offered again immediately")
			steps, _, _ = planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", after.DueAt.Add(time.Second))
...
func TestComponentRecallPlanUsesSameCooldownAsSaving(t *testing.T) {
	now := time.Date(2026, 9, 12, 12, 0, 0, 0, time.UTC)
	lastRecall := now.Add(-2 * time.Minute)
	pastDue := now.Add(-time.Minute)
	futureDue := now.Add(24 * time.Hour)
	for _, due := range []*time.Time{&pastDue, &futureDue} {
		entry := interfaces.ComponentEntry{
			ID: "difficult", Available: true,
			Material: types.ComponentDefinition{Key: "difficult", Title: "需要继续理解的目标", Minutes: 2},
			State: interfaces.ComponentState{Level: "review", Reads: 1, SelfReport: "difficult", DueAt: due,
				LastRecallAt: lastRecall, LastDifficultyAt: now.Add(-time.Minute), LastReadAt: now},
		}
		steps, _, _ := planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", now)
		require.Empty(t, steps, "do not recommend a recall that the save endpoint would reject as too recent")
		ready := lastRecall.Add(componentRecallCooldown)
		steps, _, _ = planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", ready)
		require.Len(t, steps, 1, "a new difficulty can receive recall support after the cooldown even if an older recall was scheduled later")
		require.Equal(t, "recall", steps[0].Action)
		entry.Available = false
		steps, _, _ = planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", ready)
		require.Empty(t, steps, "invalid sources still gate all recommended actions")
	}
}

func TestComponentReadWithoutDifficultyDoesNotForceRecallWhenChecksMissing(t *testing.T) {
	entry := interfaces.ComponentEntry{ID: "read", Available: true,
		Material: types.ComponentDefinition{Key: "read", Title: "已阅读目标", Minutes: 2},
		State:    interfaces.ComponentState{Level: "learning", Reads: 1},
	}
	steps, _, _ := planComponentsDetailed([]interfaces.ComponentEntry{entry}, 5, "", time.Now())
	require.Empty(t, steps, "missing checks alone do not force an extra self-report task")
}
...
func TestComponentPlannerOfflineEvaluation(t *testing.T) {
	...
	for _, file := range []string{"component_plan.go", "component_plan_test.go", filepath.Join("testdata", "component-plan-protocol.md")} {
		content, err := os.ReadFile(file)
		require.NoError(t, err)
		hash := sha256.Sum256(content)
		hashes[filepath.Base(file)] = fmt.Sprintf("%x", hash)
	}
	report := map[string]any{"protocol": "component-plan-protocol.md", "sha256": hashes, "seed": 20260912, "small_trials": trials, "strict_improvements": improved, "greedy_total_regret": greedyRegret, "search_total_regret": 0, "scale": scale, "scope": "algorithmic synthetic evaluation under declared utility; not measured learning gains"}
