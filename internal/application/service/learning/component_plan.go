...
const componentPolicyVersion = "kc-goal-progression-v3"
...
func componentCandidateStep(e interfaces.ComponentEntry, goal string, now time.Time) (interfaces.ComponentStep, bool) {
	...
	s := e.State
	recallReady := s.LastRecallAt.IsZero() || now.Sub(s.LastRecallAt) >= componentRecallCooldown
	...
	if s.DueAt != nil && !s.DueAt.After(now) && recallReady {
		...
		if len(s.ReadCheckIDs) >= len(e.Material.Checks) {
			// A completed reread need not clear uncertainty. When no new check
			// exists, offer the existing recall flow once for this difficulty;
			// subsequent reviews follow the real recall record's schedule.
			if s.Level != "review" || !recallReady || (!s.LastRecallAt.IsZero() && !s.LastRecallAt.Before(s.LastDifficultyAt)) {
				return step, false
			}
			step.Action, step.Minutes, step.Utility = "recall", 1, 1200
			step.Reason = "当前没有新检查，先用自己的话回忆要点，再对照解释；若仍有困难，可结合来源向知识助手提问。回忆反馈用于安排复习，不增加客观检查证据。"
		} else {
...
func componentCandidates(entries []interfaces.ComponentEntry, goal string, now time.Time) []componentCandidate {
	...
	include = func(key string) {
		...
		for _, p := range e.Material.Prerequisites {
			// Reading preparation is enough to continue to the target. An
			// optional check on a prepared prerequisite is not part of the
			// necessary reading closure and must not become a hidden exam.
			if prerequisite, exists := byKey[p]; exists && componentPrepared(prerequisite) {
				continue
			}
			include(p)
		}
	...
	if goal != "" {
		for _, e := range entries {
			if e.ID == goal {
				step, _ := componentCandidateStep(e, goal, now)
				if step.Action == "read" {
					include(e.Material.Key)
				} else {
					// Checks and recall revisit an already contacted target. They
					// do not require rereading its entire prerequisite curriculum.
					allowed[e.Material.Key] = true
				}
			}
		}
	} else {
	...
	for key, index := range indices {
		if out[index].step.Action != "read" {
			// Revisit only this target, but retain structural/source guards:
			// an unknown or unavailable prerequisite is never silently ready.
			for _, prerequisite := range byKey[key].Material.Prerequisites {
				p, exists := byKey[prerequisite]
				if !exists || !p.Available || prerequisite == key {
					out[index].blocked = true
				}
			}
			out[index].step.Reason += " 本次处理已有接触的目标，不要求重做前置，也不会补记前置能力。"
			continue
		}
		seen := map[int]bool{}
		for _, prerequisite := range byKey[key].Material.Prerequisites {
			...
			if exists && componentPrepared(p) {
				if p.State.Level != "familiar" {
					out[index].step.Reason += " 前置「" + p.Material.Title + "」只有阅读、自评或少量检查依据，可继续通读，不代表前置能力已通过。"
				}
				continue
			}
...
func searchComponentPlan(c []componentCandidate, budget int) ([]interfaces.ComponentStep, int, interfaces.ComponentPlanInfo) {
	...
	if len(steps) == 0 {
		info.Notice = "当前准备条件与内部计算上限下暂无可执行步骤；可以选择其他模块，或从星图回看材料、尝试回忆。已有回忆记录按复习时间安排。"
	}
	return steps, best.cost, info
}

func planComponentGoal(entries []interfaces.ComponentEntry, budget int, goal string, now time.Time) ([]interfaces.ComponentStep, int, interfaces.ComponentPlanInfo) {
	if goal != "" {
		for _, e := range entries {
	...
	if goal != "" && len(steps) > 0 {
		...
		if !included && !prepared {
			info.Notice = "本轮只安排能完成的前置步骤，尚未覆盖目标本身；完成后按实际阅读与反馈继续安排。"
		}
	}
...
func planComponentsDetailed(entries []interfaces.ComponentEntry, budget int, goal string, now time.Time) ([]interfaces.ComponentStep, int, interfaces.ComponentPlanInfo) {
	return planComponentsWithTopic(entries, budget, goal, "", now)
}
