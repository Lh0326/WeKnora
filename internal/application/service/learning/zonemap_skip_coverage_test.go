...
// TestZoneMapSkipCountsAsLit（用户反馈 4 回归）：覆盖计数语义是（已点亮）/
// （总数），"已掌握，移除推荐"是用户自己的覆盖声明——必须计入已点亮：
// 跳过一个已点亮的节点不能再让 1/1 变 0/1，跳过一个未接触的节点应把
// 0/1 变成 1/1。头部进度统计（GetProgress 的 LitNodes 与单元 Lit）与
// 分区计数同口径联动，不再"统计不同步"。
