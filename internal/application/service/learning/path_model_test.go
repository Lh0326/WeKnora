...
func TestModelCheckIsInfrequentOptionalAndBudgeted(t *testing.T) {
	...
	if len(plan.Steps) != 2 || plan.Steps[0].Reason.Code != "plan_reason_model_check" || plan.Steps[1].Slug != "b" {
		t.Fatalf("one optional check should leave reading available: %+v", plan)
	}
