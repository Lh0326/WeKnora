...
// ShortPath derives the caller's bounded path (pure planner under the
...
func planToTransport(p PathPlan) *interfaces.LearningPathPlan {
	out := &interfaces.LearningPathPlan{PolicyVersion: p.PolicyVersion, Degrade: p.Degrade, Personalization: p.Personalization, Steps: []interfaces.LearningPathStep{},
		FocusSlug: p.FocusSlug, FocusTitle: p.FocusTitle, Progression: p.Progression, Limitations: p.Limitations}
...
func (s *Service) planInputOf(ctx context.Context, kbID string, req ColdStartRequest) (planInput, error) {
	...
	if len(req.GoalObjectives) > 0 {
		// Explicit verification objectives remain the selected goals, rather
		// than falling through to unrelated page exploration if they cannot
		// proceed. Their actual prerequisites can still cross this scope.
		explicitPages := map[string]bool{}
		for _, id := range req.GoalObjectives {
			slug := pubByID[id].Slug
			if len(pageScope) > 0 && !pageScope[slug] {
				return planInput{}, fmt.Errorf("%w: objective is outside the selected page scope", ErrInvalidLearningRequest)
			}
			explicitPages[slug] = true
		}
		pageScope = explicitPages
	}
...
// docOrderRanks builds a source-position fallback rank, not a prerequisite
// or difficulty judgement.
