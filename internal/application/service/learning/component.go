...
func componentAvailable(c types.LearningComponent, pages []*types.WikiPage) bool {
	for _, ref := range c.Definition.Sources {
		found := false
		for _, p := range pages {
			if p != nil && p.TenantID == c.TenantID && p.KnowledgeBaseID == c.KnowledgeBaseID && p.Status != types.WikiPageStatusArchived && p.Slug == ref.Slug && componentHash(componentNormalize(p.Content)) == ref.Hash {
				found = true
				break
			}
		}
...
func (s *Service) ComponentView(ctx context.Context, kb string, budget int, goal string) (*interfaces.ComponentView, error) {
	...
	result.Steps, result.UsedMinutes, result.Plan = planComponentsDetailed(result.Components, budget, goal, now)
	return result, nil
}
	...
	err = s.repo.WithSubject(ctx, scope.SubjectID, epoch, true, func(tx context.Context) error {
		...
		case "recall":
			...
			for _, e := range current {
				var f componentFact
				_ = json.Unmarshal(e.ReviewData, &f)
				if f.Action == "recall" && now.Sub(e.OccurredAt) < 10*time.Minute {
