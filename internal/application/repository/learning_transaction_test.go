...
func postgresLearningTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, err)
	schema := "codex_learning_" + strings.ReplaceAll(uuid.NewString(), "-", "")
...
func TestPostgresProjectionMigrationUpDownPreservesRecords(t *testing.T) {
	db := postgresLearningTestDB(t)
	down, err := os.ReadFile(filepath.Join("..", "..", "..", "migrations", "versioned", "000091_learning_projection_identity.down.sql"))
	require.NoError(t, err)
	up, err := os.ReadFile(filepath.Join("..", "..", "..", "migrations", "versioned", "000091_learning_projection_identity.up.sql"))
	require.NoError(t, err)
